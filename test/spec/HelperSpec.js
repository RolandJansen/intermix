...
describe('A Helper', function() {
  ...
  describe('.createNoteEvent', function() {
    ...
    beforeEach(function() {
      ...
      this.value = 60;
      this.velocity = 1;
      this.msg = { type: 'note', value: this.value, velocity: this.velocity };
    ...
    describe('on success', function() {
      ...
      it('returns a valid note event when given a midi note number', function() {
        var evt = this.helper.createNoteEvent(this.uid, this.value, this.velocity);
        expect(evt).toEqual(this.evt);
      });

      it('returns a valid note event when given a tone string', function() {
        var evt = this.helper.createNoteEvent(this.uid, 'c4', this.velocity);
        expect(evt).toEqual(this.evt);
      });
